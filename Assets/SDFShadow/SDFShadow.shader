...
//        _MainTex ("Texture", 2D) = "white" {}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float3 sdfUV = uvw;
                return tex3Dlod(_SDFTex,float4(sdfUV,0)).r;
            }
            bool Inside01(float3 uvw)
            {
                return all(uvw >= 0) && all(uvw <= 1);
            }
            float SDF(float3 p,float3 L)
            {
                float depth = 0;
                float3 low = _Low-_padding;
                float3 up = _Up+_padding;
                 float res = 1.0;
                [loop]
                for (int i =0;i<_MaxStep;i++)
                {
                    ...
                    float3 uvw = p + depth*L;// 根据sdf 距离查找 进行下次步进的距离映射
                    uvw  = (uvw-low)/(up-low);// 映射到0-1
                    float dist = GetSDFWorldPos(uvw);
                    if (dist < _MinDist)
                    {
                        return 0;
                    }
