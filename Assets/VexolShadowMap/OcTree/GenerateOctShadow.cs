...
public class GenerateOctShadow : MonoBehaviour
{
    ...
    public bool DebugDrawWhite = true;
    ...
    public void GenerateTree()
    {
        ...
        _bound.size = new Vector3(maxSize, maxSize, maxSize);
        Debug.Log(_bound.size);
        _bound.center = new Vector3(maxSize / 2, maxSize/2, maxSize / 2);
        // _bound.center = _bound.center / 2 * 2;
        tree = new OcTree(_bound,maxDepth);
        fillTreeData();
        CombineNodes();
    ...
    private void ComputeRay(OcNode node)
    {
    ...
    public void CreateTex()
    {
        ...
        for (int i = 0; i < tree.Nodes.Count; i++)
        {
            ...
            colors[i].b = tree.Nodes[i].y / 255.0f;
            colors[i].b = tree.Nodes[i].children != null ? tree.Nodes[i].children[0].index/255.0f : -1;
            //如果考虑保存oc tree 的索引和状态  可以考虑将其保存到 A通道的百位值(*100)  和 个位值  因为Alpha 的精度很高 所以可以这么玩
    ...
    private void OnDrawGizmos()
    {
        ...
        if (debugMode)
        {
            tree.root.GizmosBound(0,-_light.transform.forward,DebugDrawWhite);
            ...
            if (DebugIndex!=-1)
            {
                ...
                Gizmos.DrawWireCube(tree.Nodes[DebugIndex].bound.center,tree.Nodes[DebugIndex].bound.size);
            }
